...
$(document).ready(function(){
    ...
    columns: [
      ...
      { data: null, orderable: false, render: function(row){
        ...
        row.roles.forEach(function(role){
          let $role_name = role.alias != null ? role.alias : role.name
          $roles += "<span class='badge bg-primary me-1'>"+$role_name+"</span>"
        })

        if(row.company_id != null && row.is_super){
          $roles += "@<sapn class='badge bg-success ms-1'>"+row.company.name+"</span>"
        }
        return $roles;
      }},
  ...
  $('#role').on('change', function(){
    ...
    if($value != 1){
      $('#select_company').slideDown();
    }else{
      $('#select_company').slideUp();
    }
  })
  ...
  $.ajax({
    ...
    success: function(response){
      if(response.status){
        ...
        $('#company').val(user.company_id).change();
