        ...
        <div class="card">
            ...
            <div class="card-header border-bottom p-1">
                <button type="button" class="btn btn-outline-primary waves-effect"
                 ...
                 $('#formNewSalary').attr('action', '{{url('employee/'.request()->segment(2).'/employee-salary')}}');
                 $('.reset').click();
                 $('#addNewSalaryLabel').html('{{lang('Employees.Add New Salary')}}');
                    ...
                    @endphp
                        <tr>
                            ...
                            <td>
                                ...
                                onclick="
                                    $('#addNewSalaryLabel').html('{{lang('Employees.Edit Salary')}}');
                                    $('#formNewSalary').attr('action', '{{url('employee/'.request()->segment(2).'/employee-salary/'.$item->id)}}');
                                    $('.method').attr('name', '_method');
                                    $('.form-control').removeClass('is-invalid');
                                    $('.errs').remove();
                                    
                                    $('[name=salary_id]').val('{{$item->salary->id}}').trigger('change');
                                ...
                                <button class="btn btn-icon btn-flat-danger rounded-circle waves-effect btn-delete" target_url="{{url('employee/'.request()->segment(2).'/employee-salary/'.$item->id)}}" id_data="{{$item->id}}">
          ...
          <div class="modal-body">
            ...
            <div class="row">
                <div class="col-md-12 form-group mb-1">
                    ...
                    <select name="salary_id" id="salary_salary" required="" class="form-control select2">
